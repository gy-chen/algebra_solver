...
import {
  ...
  POLL_TASK,
  UpdateTaskAction,
  TaskEpicDependencies,
  PolledTaskAction,
  SubmitTaskErrorAction,
  TaskContentErrorType
} from "./types";
import { RootState } from "../types";

type SubmitTaskEpicActions =
  | PollTaskAction
  | SubmittedTaskAction
  | SubmitTaskErrorAction;

export const submitTaskEpic = (
  action$: ActionsObservable<SubmitTaskAction>,
  state$: StateObservable<RootState> | null,
  { taskApi }: TaskEpicDependencies
) =>
  action$.pipe(
    ofType(SUBMIT_TASK),
    switchMap(
      (action): Observable<SubmitTaskEpicActions> => {
        return Observable.create(
          (observer: Observer<SubmitTaskEpicActions>) => {
            taskApi
              .createTask(action.payload.content)
              .then(res => {
...
export const pollTaskEpic = (
  action$: ActionsObservable<PollTaskAction>,
  state$: StateObservable<RootState> | null,
  { taskApi }: TaskEpicDependencies
) =>
  action$.pipe(
    ofType(POLL_TASK),
    switchMap(
      (action): Observable<PollTaskEpicActions> => {
        return Observable.create((observer: Observer<PollTaskEpicActions>) => {
          taskApi.pollTask(action.payload.id).subscribe(
            task => {
...
export const taskEpic = combineEpics(submitTaskEpic, pollTaskEpic);
