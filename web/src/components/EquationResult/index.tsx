...
const Container = styled.div`
  animation: pop-up 0.5s cubic-bezier(0, 0.98, 0.49, 1.19) forwards;
...
const VariableRowHead = styled.div`
  ...
  padding: 0.4em 0.5em;
  text-align: right;
  background-color: rgb(224, 225, 226, 0.4);
`;
...
const ResultContainer = styled.div`
  ...
  :before {
    display: ${(p: ResultContainerProps) =>
      p.hasLargeLoss ? "unset" : "none"};
    content: "";
    position: absolute;
    left: 0;
    right: 0;
...
const EquationResult = (props: EquationResultProps) => {
  ...
  let variableRows = _.keys(task.result)
...
const renderVariableRow = (
  ...
  return (
    <RowContainer key={variable}>
      ...
      <RowContent>
        {typeof result === "number" ? result.toPrecision(2) : result}
      </RowContent>
    </RowContainer>
  );
};
